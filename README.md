...
点击顶部「老门东」→「一键演示」→「领演示券」→「去商家台核销」。影像工作台可一键生成当前场景的 2:1 合成全景，拖动取景并导出横版/竖版静态图。样例不冒充 X4 Air 实拍。
...
静态 Sites 展示不运行 Node 后台，预置示例、本地意图分类、合成全景、券和看板均可直接使用。真实素材服务和实时模型需要独立云服务器部署。演示券仅存在当前浏览器 localStorage，不能作为多终端生产账本。
