...
export function createWorld(container, onSelect){
 ...
 function resize(){const w=Math.max(1,container.clientWidth),h=Math.max(1,container.clientHeight);renderer.setSize(w,h);camera.aspect=w/h;camera.fov=mode==='asset'&&panorama?60:w/h<.9?55:39;camera.updateProjectionMatrix();post.resize(w,h);}new ResizeObserver(resize).observe(container);resize();
 ...
 function showReferenceModel(){clearAsset();referenceModel=createGateway();assets.add(referenceModel);setMode('asset');camera.fov=camera.aspect<.9?55:39;camera.updateProjectionMatrix();controls.maxDistance=180;controls.minDistance=5;assetCredit={title:'印度门 · 照片参考模型'};assetIsDemo=false;fly([45,32,58],[0,10,0]);}
