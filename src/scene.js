...
export function createWorld(container, onSelect){
 ...
 const leaves=new THREE.InstancedMesh(new THREE.IcosahedronGeometry(1,2),leaf,trees.length);const trunks=new THREE.InstancedMesh(new THREE.CylinderGeometry(.17,.3,1,5),trunk,trees.length);const tmp=new THREE.Object3D();trees.forEach(([x,z,h],i)=>{tmp.position.set(x,h*.8,z);tmp.scale.set(h*.54,h*.64,h*.54);tmp.rotation.set(0,random()*6,0);tmp.updateMatrix();leaves.setMatrixAt(i,tmp.matrix);leaves.setColorAt(i,new THREE.Color().setHSL(.28+random()*.04,.18+random()*.09,.28+random()*.15));tmp.position.y=h*.23;tmp.scale.set(1,h*.8,1);tmp.updateMatrix();trunks.setMatrixAt(i,tmp.matrix);});leaves.castShadow=true;leaves.receiveShadow=true;resort.add(leaves,trunks);
