...
export function upgradeResort(root,lakeGeometry,oldLake){
 ...
 const loop=[];for(let a=0;a<Math.PI*2;a+=.15)loop.push([Math.cos(a)*84,.35,Math.sin(a)*75+11]);const people=visitors(root,[makePath(loop,true),makePath([[-30,.4,0],[0,.4,9],[31,.4,0],[0,.4,9],[-30,.4,0]])],{count:24,seed:37});
