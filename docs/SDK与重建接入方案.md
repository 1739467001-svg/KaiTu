...
设计原则：用户只看到“拍摄一个区域 → 检查并处理 → 校准与交付 → 接入空间服务”；SDK 控制、拼接、位姿求解和高斯训练在采集端/处理端分工，统一由一个场景清单连接。
...
- AirSim360 适用于无人机仿真/数据生成，不是本次场地采集 MVP 的必要依赖。
...
`server/app.mjs` 提供鉴权场地、素材和任务接口；`server/worker.mjs` 实际执行 FFmpeg 检查与抽帧。原始 INSV 返回需拼接状态。原生 SDK、AI 推理与 GPU 训练仍需独立适配，并非已经包含在 CPU worker 内。具体部署见《云部署与体系架构》。
